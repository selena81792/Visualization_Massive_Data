...
def plot_pie(my_data_choice, my_parameter_choice):
    ...
    new_row = pd.DataFrame(data = {
        'Country' : ['Others'],
        my_data_choice : [df[my_data_choice][my_parameter_choice:].sum()]
    })
    df2 = pd.concat([df2, new_row])
    plt.pie(df2[my_data_choice],
        labels=df2['Country'],
        autopct='%1.1f%%',
    )
    plt.axis('equal')
...
plot_pie(my_data_choice_words(my_data_choice_number), my_parameter_choice)